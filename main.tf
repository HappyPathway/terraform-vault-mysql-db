...
resource "vault_database_secret_backend_connection" "mysql" {
  ...
  mysql {
    connection_url = "${data.vault_generic_secret.db_credentials.data["username"]}:${data.vault_generic_secret.db_credentials.data["password"]}@tcp(${var.db_host}:3306)/${data.terraform_remote_state.db.db_name}"
  }
}
